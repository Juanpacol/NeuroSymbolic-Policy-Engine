...
| `h1_h3/ablations.json` | `python -m nspe.ablate.cli --clip-model ViT-L-14 --cache-dir ./emb_cache --device cuda --configs linear_probe --seeds 0 1 2` then `--seeds 0..9` (all configs) -- 70 rows, 7 configs x 10 seeds. Supersedes an earlier 3-seed, 6-config version (`linear_probe` was added to the code but never run until this session). | Kaggle T4 |
| `h1_h3/results_val_s{0..9}.json` | Held-out-test rerun's validation pass -- fresh checkpoints, same protocol as `results_s*.json` above, 10 seeds so the exact significance test can reach p<0.05 (impossible at n=5). Fits the thresholds `results_test_s*.json` applies. Supersedes an earlier 5-seed `s{0..4}` version of this same file set. | Kaggle T4 |
| `h1_h3/results_test_s{0..9}.json` | `python -m nspe.eval.cli --clip-model ViT-L-14 --cache-dir ./emb_cache --split test --thresholds-from results_val_s{i}.json` (one per seed; `--split test` refuses to run without `--thresholds-from`) | Kaggle T4 |
| `h1_h3/results_scram_val_s{0..9}.json`, `h1_h3/results_scram_test_s{0..9}.json` | The scrambled-policy control: reasoner retrained per policy in `h1_h3/policies_scrambled/`, baseline reused from the intact run above. See `docs/h1_h3_findings.md`'s "Scrambled-policy control" section for the result -- **note the glob hazard**: `results_s*.json` also matches `results_scram_test_s0.json` (`results_s` + `cram_test_s0`); use `results_s[0-9].json` or a directory-scoped tool call to avoid silently pooling 5 seeds with 25. | Kaggle T4, same session as the val/test rerun above |
| `h1_h3/results_zs_val_s{0..9}.json`, `h1_h3/results_zs_scram_val_s{0..9}.json` | The zero-shot-only diagnostic: `python -m nspe.train.cli --model reasoner --epochs 0 ...` (intact and each scrambled policy), evaluated on validation only. Isolates whether the scrambled-policy control's null result comes from training erasing an initial wiring advantage, or from there being none to begin with. See `docs/h1_h3_findings.md`'s "Zero-shot-only diagnostic" section. | Kaggle T4 |
| `h1_h3/mu0_val_s{0..9}.json` | Trained base predicate degrees plus labels, from `nspe.eval.cli --dump-mu0` on the intact policy (`docs/colab_h1_h3.md` Cell 12). **An input, not a result** — but committed on purpose: it is what makes the wiring sweep's 7200 AUROC numbers checkable and re-runnable by a reader with no GPU, the same reasoning that put `policies_scrambled/` here. ~50KB each, columns ordered by `predicate_names`, attributable via `policy_fingerprint`. | Kaggle T4 |
| `h1_h3/wiring_sweep_val.json` | `python -m nspe.eval.wiring_sweep --mu0 docs/results/h1_h3/mu0_val_s{0..9}.json --out ...` — every one of the 720 rewirings of the six base predicates scored against a frozen predicate layer, per seed. Derived entirely from the `mu0_val_s*.json` above, so it needs no GPU and reruns in seconds. | Any CPU |
